...
export const createCase = async (args: CreateCaseArgs) => {
    try {
        ...
        try {
            ...
            const fallback = "An unexpected error occured while attempting to open a new case. Please try again later"
...
export type EditCaseData = Partial<Omit<CaseMetadata, "totalExhibits" | "totalParties" | "updatedAt" | "id">>
export const editCase = async (caseId: string, data: EditCaseData) => {
    try {
        const session = await auth();
        if (!session?.user?.id) {
            return { success: false, data: null, message: "Unauthorized. Please sign in and try again" };
        ...
        const userId = session.user.id;
        try {
            const caseRecord = await prisma.caseFile.update({
                where: { id: caseId, userId },
                data,
                select: { id: true }
            });
            return { success: true, data: { caseId: caseRecord.id }, message: "Case's details have been successfully updated" }
        } catch (err) {
            const fallback = "An error occured while attempting to update this case's details. Please try again later"
            console.error("[Server Action (editCase)] Database error:", err);
            return {
                success: false,
                data: null,
                message: getPrismaErrorMessage(err, fallback)
            };
        }
    } catch (err) {
...
export const deleteCase = async (caseId: string) => {
    try {
        ...
        const userId = session.user.id;
        try {
            const deletedCase = await prisma.caseFile.delete({
                where: { id: caseId, userId },
                select: { title: true }
            });
            return { success: true, message: `"${deletedCase.title}" has been successfully deleted` }
        } catch (err) {
            const fallback = "An error occured while attempting to delete the requested case. Please try again later"
            console.error("[Server Action (deleteCase)] Database error:", err);
            return {
                success: false,
                data: null,
                message: getPrismaErrorMessage(err, fallback)
        ...
        console.error("[Server Action (deleteCase)] Error:", err);
        return { success: false, data: null, message: "Something went wrong. Please try again later" };
    }
}