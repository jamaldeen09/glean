...
import { ArrowRight, Files, Users } from "lucide-react";
import { Card, CardHeader, CardTitle, CardContent, CardFooter } from "@/components/ui/card";
import { CaseStatus } from "@/generated/prisma/client";
import { timeAgo } from "@/lib/utils";
import { Skeleton } from "../ui/skeleton";
import DeleteCaseConfirmation from "../dialogs/DeleteCaseConfirmation";
import { CaseMetadata } from "@/app/workspace/_actions";
import EditCase from "../dialogs/EditCase";
...
export function CaseCard({ case: c }: { case: CaseMetadata }) {
    return (
        <Card className="transition-shadow duration-100 h-full">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
                <span className="text-xs font-medium text-muted-foreground">
                  {!c.number ? "No number" : c.number}
                </span>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <DeleteCaseConfirmation caseId={c.id} />
                    <EditCase caseId={c.id} caseData={{
                        title: c.title,
                        court: c.court,
                        client: c.client,
                        status: c.status,
            ...
            <CardFooter className="flex items-center gap-4 text-sm text-muted-foreground bg-transparent py-2">
                <span className="inline-flex items-center gap-1.5">
                ...
                <Button size="icon-sm" className="ml-2 rounded-full">
                    <Link href={`/case/${c.id}/exhibits`}>
                        <ArrowRight />
                    </Link>
                </Button>
            </CardFooter>
