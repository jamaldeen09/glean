// hooks/use-edit-case.ts
import { useMutation, useQueryClient, type UseMutationOptions } from '@tanstack/react-query';
import { editCase } from '@/app/actions/case'; // adjust import path
import type { CaseMetadata } from '@/lib/types'; // adjust import

// Infer the server action's return type
type EditCaseResult = Awaited<ReturnType<typeof editCase>>;

// The variables our mutation will accept
type EditCaseVariables = {
  caseId: string;
  data: Partial<Omit<CaseMetadata, 'totalExhibits' | 'totalParties' | 'updatedAt'>>;
};

// Optional context for rollback/optimistic updates
...
export function useEditCase(
  options?: Omit<
    UseMutationOptions<EditCaseResult, Error, EditCaseVariables, MutationContext>,
    'mutationFn'
  >
) {
  const queryClient = useQueryClient();

  return useMutation<EditCaseResult, Error, EditCaseVariables, MutationContext>({
    mutationFn: async ({ caseId, data }) => {
      return editCase(caseId, data);
    },
