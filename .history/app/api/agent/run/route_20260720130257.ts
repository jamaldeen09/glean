...
export async function POST(req: NextRequest) {
    try {
        ...
        if (typeof caseId !== "string" || typeof chatSessionId !== "string") {
            return NextResponse.json(
                { success: false, message: "caseId and chatSessionId are required." },
                { status: 400 }
            );
        }
        ...
        const caseRecord = await prisma.caseFile.findFirst({
        ...
        const result = await runAgent({ caseId, chatSessionId, userMessage: trimmedMessage });
