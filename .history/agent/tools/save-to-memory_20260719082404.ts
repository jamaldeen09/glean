...
export const saveToMemory = tool(
    async ({ content }, runtime) => {
        try {
            const docketId = runtime?.configurable?.docketId;
            if (!docketId) throw new Error("Missing docket scope in tool context.");
          ...
          INSERT INTO "MemoryFragment"
            (id, docket_id, content, embedding, access_count, last_used_at, created_at, updated_at)
          VALUES
            (${createId()}, ${docketId}, ${content}, ${JSON.stringify(embedding)}::vector, 0, now(), now(), now());
        `;
