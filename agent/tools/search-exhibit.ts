...
export const searchExhibit = tool(
    async ({ queryText, exhibitId, limit }, config) => {
        try {
            const docketId = config?.configurable?.docketId;
            if (!docketId) throw new Error("Missing docket scope in tool context.");

            // Verify the exhibit actually belongs to this docket before searching it.
            const exhibit = await prisma.exhibit.findFirst({
                where: { id: exhibitId, docketId },
                select: { id: true },
            });
